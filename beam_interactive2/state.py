...
class State(EventEmitter):
    ...
    async def get_scenes(self):
        """
        Can be called by the existing State to retrieve existing Scenes and their controls and groups.
        """
        return await self._connection.call('getScenes', {})

    def set_scenes(self, scenes):
        """
        Can be called with one or more Scenes to add them to the State.
        This does NOT create scenes and add them into the Interactive.
        Use if you need to update the interactive state based upon the existing Interactive State.
        :param scenes: json of scenes and their controls
        :type scenes: json
        """
        sid = []
        for s in scenes['scenes']:
            sid.append(Scene(s['sceneID']))
        for scene in sid:            
            self._scenes[scene.id] = scene
            scene._attach_connection(self)
